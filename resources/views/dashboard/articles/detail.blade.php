        ...
        <div class="p-3 md:p-6">
            <div class="card bg-base-100 md:w-2/3 shadow-xl">
                <div class="card-body">
                    ...
                    <form action="{{ route('articles.edit-article', ['slug' => $article->slug]) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        ...
                        <label class="form-control w-full">
                            ...
                            <input type="hidden" name="categories" id="categories-input-hidden" value="{{ implode(",", $article->categories) }}">
                            <select class="select select-bordered" id="categories" multiple>
                                @foreach ($categories as $category)
                                    <option @if(in_array($category->category_name, $article->categories)) @selected(true) @endif>{{ $category->category_name }}</option>
                                @endforeach
                            </select>
                            <div class="label">
                                @error('categories')
                        ...
                        <label class="form-control w-full">
                            ...
                            <input type="file" name="thumbnail" id="thumbnail" class="file-input file-input-bordered w-full" value="{{ old('thumbnail') }}" onchange="previewThumbnail()" />
                        ...
                        <form action="{{ route('articles.delete-article', ['slug' => $article->slug]) }}" method="post">
                            @csrf
                            @method("DELETE")
                            <button type="submit" onclick="return confirm('Sure delete this article?')" class="btn bg-red-500 hover:bg-green-300 text-white">Delete</button>
                        </form>
                        </div>
                </div>
            </div>
